...
public:
  ...
  // end points
  Spline() : _valid(false), _BCLow(FIXED_2ND_DERIV_BC), _BCHigh(FIXED_2ND_DERIV_BC), _BCLowVal(0), _BCHighVal(0), _type(CUBIC) {}
  ...
  double operator()(double xval) {
    ...
    for (std::vector<SplineData>::const_iterator iPtr = _data.begin(); iPtr != _data.end() - 1; ++iPtr)
    ...
    switch (_type) {
      ...
      case CUBIC: {
        ...
        for (size_t yv(0); yv <= e; ++yv)
          for (size_t xv(0); xv <= e; ++xv) A(xv, yv) = 0;
        ...
        for (size_t xv(0); xv <= e; ++xv) C(xv) = 0;

        for (size_t i(1); i < e; ++i) C(i) = 6 * ((y(i + 1) - y(i)) / h(i) - (y(i) - y(i - 1)) / h(i - 1));
