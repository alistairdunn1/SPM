...
//**********************************************************************
void CBinomialwithQLikelihood::getResult(vector<double>& scores, const vector<double>& expected, const vector<double>& observed, const vector<double>& errorValue,
  ...
  for (int i = 0; i < (int)expected.size(); ++i) {
    ...
    double dScore      = CMath::lnFactorial(dErrorValue) - CMath::lnFactorial(dErrorValue * (1.0 - observed[i])) - CMath::lnFactorial(dErrorValue * observed[i])
                    + dErrorValue * observed[i] * log(CMath::zeroFun(expected[i], delta)) + dErrorValue * (1.0 - observed[i]) * log(CMath::zeroFun(1.0 - expected[i], delta));
...
//**********************************************************************
void CBinomialwithQLikelihood::simulateObserved(const vector<string>& keys, vector<double>& observed, const vector<double>& expected, const vector<double>& errorValue,
                                                const vector<double>& processError, const double delta) {
