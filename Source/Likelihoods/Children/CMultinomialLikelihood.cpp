...
//**********************************************************************
void CMultinomialLikelihood::getResult(vector<double>& scores, const vector<double>& expected, const vector<double>& observed, const vector<double>& errorValue,
  ...
  for (int i = 0; i < (int)expected.size(); ++i) {
    ...
    double dScore      = CMath::lnFactorial(dErrorValue * observed[i]) - dErrorValue * observed[i] * log(CMath::zeroFun(expected[i], delta));
...
//**********************************************************************
void CMultinomialLikelihood::simulateObserved(const vector<string>& keys, vector<double>& observed, const vector<double>& expected, const vector<double>& errorValue,
                                              const vector<double>& processError, const double delta) {
  ...
  for (int i = 0; i < (int)vUniqueKeys.size(); ++i) {
    ...
    for (int j = 0; j < (int)vExpected.size(); ++j) {
      ...
      if (j > 0)
        vExpected[j] = vExpected[j] + vExpected[j - 1];
    }
...
// Get Initial Score
//**********************************************************************
double CMultinomialLikelihood::getInitialScore(const vector<string>& keys, const vector<double>& expected, const vector<double>& observed, const vector<double>& processError,
