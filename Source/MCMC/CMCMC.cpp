...
void CMCMC::build() {
  try {
    ...
    foreach (CEstimate* Estimate, vEstimates) { vEstimateNames.push_back(Estimate->getParameter()); }
...
bool CMCMC::choleskyDecomposition() {
  ...
  for (int i = 1; i < iN; ++i) mxCovarianceLT(i, 0) = mxCovariance(i, 0) / mxCovarianceLT(0, 0);

  for (int i = 1; i < iN; ++i) {
    dSum = 0.0;
    for (int j = 0; j < i; ++j) dSum += mxCovarianceLT(i, j) * mxCovarianceLT(i, j);
    ...
    for (int j = i + 1; j < iN; ++j) {
      dSum = 0.0;
      for (int k = 0; k < i; ++k) dSum += mxCovarianceLT(j, k) * mxCovarianceLT(i, k);
      mxCovarianceLT(j, i) = (mxCovariance(j, i) - dSum) / mxCovarianceLT(i, i);
    }
  }

  dSum = 0.0;
  for (int i = 0; i < (iN - 1); ++i) dSum += mxCovarianceLT(iN - 1, i) * mxCovarianceLT(iN - 1, i);
