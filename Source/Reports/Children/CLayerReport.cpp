...
void CLayerReport::build() {
  try {
    ...
    if (sLayerType == PARAM_ABUNDANCE_DENSITY || sLayerType == PARAM_ABUNDANCE || sLayerType == PARAM_BIOMASS_DENSITY || sLayerType == PARAM_BIOMASS || sLayerType == PARAM_DOUBLE
        || sLayerType == PARAM_META_NUMERIC || sLayerType == PARAM_DERIVED_QUANTITY || sLayerType == PARAM_DERIVED_QUANTITY_BY_CELL) {
...
void CLayerReport::execute() {
  try {
    ...
    if (sType == PARAM_DOUBLE) {
      ...
      std::cout << "from_" << PARAM_ROW << CONFIG_SPACE_SEPARATOR << "from_" << PARAM_COLUMN << CONFIG_SPACE_SEPARATOR << "to_" << PARAM_ROW << CONFIG_SPACE_SEPARATOR << "to_"
                << PARAM_COLUMN << CONFIG_SPACE_SEPARATOR << PARAM_DISTANCE << "\n";
      for (int i = 0; i < pNumericLayer->getHeight(); ++i) {
        for (int j = 0; j < pNumericLayer->getWidth(); ++j) {
          for (int k = 0; k < pNumericLayer->getHeight(); ++k) {
            for (int l = 0; l < pNumericLayer->getWidth(); ++l) {
              std::cout << i + 1 << " " << j + 1 << " " << k + 1 << " " << l + 1 << " " << pNumericLayer->getValue(i, j, k, l) << "\n";
            }
          }
        }
      }
        ...
        for (int j = 0; j < pLatLongDistanceLayer->getWidth(); ++j) {
          for (int k = 0; k < pLatLongDistanceLayer->getHeight(); ++k) {
            for (int l = 0; l < pLatLongDistanceLayer->getWidth(); ++l) {
              std::cout << pLatLongDistanceLayer->getLong(i, j) << " " << pLatLongDistanceLayer->getLat(i, j) << " " << pLatLongDistanceLayer->getLong(k, l) << " "
                        << pLatLongDistanceLayer->getLat(k, l) << " " << pLatLongDistanceLayer->getValue(i, j, k, l) << "\n";
            }
          }
        }
      }
        ...
        for (int j = 0; j < pDijkstraDistanceLayer->getWidth(); ++j) {
          for (int k = 0; k < pDijkstraDistanceLayer->getHeight(); ++k) {
            for (int l = 0; l < pDijkstraDistanceLayer->getWidth(); ++l) {
              std::cout << i + 1 << " " << j + 1 << " " << k + 1 << " " << l + 1 << " " << pDijkstraDistanceLayer->getValue(i, j, k, l) << "\n";
            }
          }
        }
      }
        ...
        for (int j = 0; j < pLatLongDijkstraDistanceLayer->getWidth(); ++j) {
          for (int k = 0; k < pLatLongDijkstraDistanceLayer->getHeight(); ++k) {
            for (int l = 0; l < pLatLongDijkstraDistanceLayer->getWidth(); ++l) {
              std::cout << pLatLongDijkstraDistanceLayer->getLong(i, j) << " " << pLatLongDijkstraDistanceLayer->getLat(i, j) << " " << pLatLongDijkstraDistanceLayer->getLong(k, l)
                        << " " << pLatLongDijkstraDistanceLayer->getLat(k, l) << " " << pLatLongDijkstraDistanceLayer->getValue(i, j, k, l) << "\n";
            }
          }
        }
      }
