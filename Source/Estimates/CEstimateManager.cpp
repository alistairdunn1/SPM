...
void CEstimateManager::fillVector(vector<CEstimate*>& list) {
  ...
  foreach (CEstimate* Estimate, vEstimateList) { list.push_back(Estimate); }
}
...
void CEstimateManager::loadEstimateValues(int index) {
  try {
    foreach (CEstimate* Estimate, vEstimateList) { Estimate->loadValue(index); }
...
void CEstimateManager::validate() {
  try {
    ...
    if (vEstimateInfoList.size() > 0) {
      foreach (CEstimate* estimate, vEstimateList) { delete estimate; }
     ...
     */
    foreach (CEstimateInfo* info, vEstimateInfoList) { info->generateEstimates(); }
    ...
    foreach (CEstimate* Estimate, vEstimateList) { Estimate->validate(); }
...
CEstimateManager::~CEstimateManager() {
  ...
  foreach (CEstimateInfo* info, vEstimateInfoList) { delete info; }
}
