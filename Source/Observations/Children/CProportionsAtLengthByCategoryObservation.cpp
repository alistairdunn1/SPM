  ...
  try {
    ...
    if (pRuntimeController->getRunMode() == RUN_MODE_SIMULATION) {
      ...
      for (int i = 0; i < (int)vObserved.size(); ++i)
        saveComparison(vKeys[i], vBin[i], std::string(""), vExpected[i], vAdjustedObserved[i], vErrorValue[i], vProcessError[i],
      ...
      for (int i = 0; i < (int)vScores.size(); ++i) {
        ...
        saveComparison(vKeys[i], vBin[i], std::string(""), vExpected[i], vObserved[i], vErrorValue[i], vProcessError[i],
