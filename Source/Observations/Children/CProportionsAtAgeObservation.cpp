...
void CProportionsAtAgeObservation::build() {
  try {
    ...
    for (int i = 0; i < (iAgeSpread * pCategories->getNRows()); ++i) pAgeResults[i] = 0.0;
  } catch (string& Ex) {
...
void CProportionsAtAgeObservation::execute() {
  ...
  try {
    ...
    while (mvObsPtr != mvObservationMatrix.end()) {
      ...
      for (int i = 0; i < (iAgeSpread * pCategories->getNRows()); ++i) dRunningTotal += pAgeResults[i];
    ...
    if (pRuntimeController->getRunMode() == RUN_MODE_SIMULATION) {
      ...
      for (int i = 0; i < (int)vObserved.size(); ++i)
        saveComparison(vKeys[i], vAges[i], vGroup[i], vExpected[i], vObserved[i], vErrorValue[i], vProcessError[i], pLikelihood->adjustErrorValue(vProcessError[i], vErrorValue[i]),
                       1.0, 0.0);
    } else {  // Generate Score
