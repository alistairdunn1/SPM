...
#'
"spm.dplot" <- function(..., name = T, quantiles = c(0.5), plot.mean = F, main = "", xlab = "", ylab = "", ylim, srtx = 0, bw = "nrd0", adjust = 1 / 3, adj = 0, fill = F, gap = 0.2) {
  ...
  if (gap > 1) gap <- 0.2
  if (!missing(ylim))
    plot(c(xmin, xmax), c(ymin, ymax), type = "n", main = main, xlab = xlab, ylab = ylab, xaxt = "n", ylim = ylim)
  else
    plot(c(xmin, xmax), c(ymin, ymax), type = "n", main = main, xlab = xlab, ylab = ylab, xaxt = "n")
  for (i in 1:n) {
  ...
  if (is.logical(name)) {
    ...
    axis(1, centers, name, srt = srtx, adj = if (srtx == 0) adj else 1)
  invisible(centers)
}
