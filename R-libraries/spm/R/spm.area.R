#' utility function
#'
#' @author Alistair Dunn (based on code by Ian Doonan)
#'
"spm.area" <- function(corners) {
  # Ian Doonan
  ...
  for (j in c(I0:(n))) {
    ...
    if (i1 > n)
      i1 <- I0
    if (is.na(corners$y[i1]) | is.na(corners$x[i1]))
      i1 <- I0 # still problems if 2 NA's in a row
