...
\defSub{removals} {Request a summary of requested and actual removals in a mortality process}
\defType{Switch}
\defDefault{False}
\defValue{Generates vectors of the total annual requested and actual removals for an Event Mortality Process or a Biomass Event Mortality Process. For other processes, this switch has no effect}
...
\defType{Switch}
\defDefault{True}
\defValue{Either True or False. Note that the option False will overwrite sequential simulations if using multiple lines in the \texttt{-i} file}
...
\defCondition{Only available if the age\_size relationship c.v.s are defined by length}
