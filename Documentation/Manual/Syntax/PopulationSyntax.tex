...
\defType{Constant}
\defDefault{0.0}
\defValue{Defines the growth proportion for calculating size and weight in that time-step}
...
\subsubsection[None]{\commandlabsubarg{process}{type}{none}}

There are no other subcommands for \commandlabsubarg{process}{type}{none}.
...
\subsubsection[Beverton-Holt recruitment process parameterised by YCS]{\commandlabsubarg{process}{type}{bh\_recruitment}}
...
\defDefault{No default}
\defValue{The expanded vector must have values of years between \commandsub{model}{initial}-offset and \commandsub{model}{current}-offset}
...
\subsubsection[Beverton-Holt recruitment process parameterised by absolute annual recruitments]{\commandlabsubarg{process}{type}{bh\_recruitment2}}
...
\defType{Integer vector or integer range}
\defDefault{No default}
\defValue{The expanded vector must have values of years that are members of }

\defSub{standardise\_recruitment\_years} {Years from which recruitments are used to derive $R_0$}
\defType{Integer vector or integer range}
\defDefault{No default}
\defValue{The expanded vector must have values of years that are members of \texttt{years}}

\defSub{layer} {Name of the layer used to determine where recruitment occurs}
\defType{String}
\defDefault{No layer}
\defValue{A valid layer as defined by \command{layer}. If a numeric layer, then the total recruitment in each cell is scaled to be proportional to the value of the layer in that cell.}
...
\subsubsection[Age mortality rate process]{\commandlabsubarg{process}{type}{age\_mortality\_rate}}

\defSub{m} {Define the age specific mortality rate to be applied}
\defType{Estimable vector, of length equal to the number of age classes in the model}
\defDefault{No default}
\defValue{A vector of positive real numbers}
...
\subsubsection[Knife-edge]{\commandlabsubarg{preference\_function}{type}{knife\_edge}}
...
\subsubsection[von Bertalanffy k-at-age]{\commandlabsubarg{age\_size}{type}{von\_bertalanffy\_k\_at\_age}}

\defSub{linf} {Define the $L_\infty$ parameter of the von Bertalanffy k-at-age relationship}
\defType{Estimable}
\defDefault{No default}
\defValue{A positive real number}
\defCondition{Only define if using the von Bertalanffy k-at-age relationship}
...
\defDefault{No default}
\defValue{A positive real number}
\defCondition{Only define if using the von Bertalanffy with k-at-age relationship}
...
\subsubsection[mean size-at-age]{\commandlabsubarg{age\_size}{type}{mean\_size\_at\_age}}

\defSub{sizes} {Define the mean size at age for each age class in the partition}
\defType{Estimable}
\defDefault{No default}
\defValue{A vector of positive real numbers with length the same as the number of age classes in the partition}
\defCondition{Only define if using the mean size-at-age relationship}
