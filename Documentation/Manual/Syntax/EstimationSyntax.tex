...
\defType{string}
\defDefault{asin}
\defValue{Either asin for the $\arcsin()$ transformation or tan for the $\tan()$ transformation}
...
\subsection{Defining the parameters to be estimated and their priors}\index{Defining the parameters to be estimated}\index{Parameter priors}
...
\defDefault{No default}
\defValue{Defines the names of all the other parameters which are constrained to have the same value as this parameter. Do \emph{not} put these parameters in as a separate \command{estimate} block as well.}
