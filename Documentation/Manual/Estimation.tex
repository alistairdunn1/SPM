...
\subsection{\I{Priors}\label{sec:priors}}
...
\subsection{\I{Penalties}\label{sec:penalties}}
...
For the penalties, you need to specify a multiplier, and the objective function is increased by this multiplier times the penalty value as described below. In some cases you will need to make the multiplier quite large to prohibit some model behaviour. 

Currently, the processes \commandlabsubarg{process}{type}{event\_mortality}, \commandlabsubarg{process}{type}{biomass\_event\_mortality}, \commandlabsubarg{process}{type}{category\_transition}, \commandlabsubarg{process}{type}{category\_transition\_by\_age}, 
\commandlabsubarg{process}{type}{holling\_mortality\_rate}, and \commandlabsubarg{process}{type}{prey\_suitability\_predation} allow penalties. Penalties for \commandlabsubarg{process}{type}{event\_mortality} and \commandlabsubarg{process}{type}{biomass\_event\_mortality} are compulsory.

For processes that have penalties, two types of penalty can be defined, natural scale (the default) and log scale. Both of these types add a penalty value of the squared difference between the observed value (e.g., the actual number of individuals to be removed in an event mortality process or the actual number of individuals to shift in a category transition process), and the number that were moved (if less than), times the penalty multiplier.

The natural scale penalty uses at the squared difference on a natural scale, while the log scale penalty uses the squared difference of the logged values. 
